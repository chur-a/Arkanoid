...
def ball():
    ...
    if keys[pygame.K_SPACE] and start_mode:
        start_mode = False
    elif start_mode == False:
        if y_ball - vy_ball < 0:
            ball_direction(vx_ball,-vy_ball)
        elif x_ball + vx_ball > 1000 or x_ball + vx_ball < 0:
            ball_direction(-vx_ball,vy_ball)
        elif x_platform - radius_ball < x_ball+vx_ball < x_platform + lenght_platform + radius_ball and y_platform <= y_ball - vy_ball + radius_ball < y_platform+10:
            angle_change()
        elif y_platform - radius_ball < y_ball < y_platform + height_platform + radius_ball:
            if x_platform - 5 < x_ball + radius_ball <= x_platform + 5 or x_platform + lenght_platform - 5 <= x_ball - radius_ball < x_platform + lenght_platform + 5:
                ball_direction(-vx_ball,vy_ball)
            else:
                ball_direction(vx_ball,vy_ball)
        else:
...
def coliisions_of_ball_rectangles():
    ...
    for j,i in Rectangles:
        ...
        if x_rect - radius_ball < x_ball < x_rect + width_rectangle + radius_ball:
            if y_rect + height_rectangle - 10 < y_ball - radius_ball <= y_rect + height_rectangle or y_rect + 10 < y_ball + radius_ball <= y_rect:
                ball_direction(vx_ball,-vy_ball)
                Delete_rectangles.append((j,i))
        elif y_rect - radius_ball < y_ball < y_rect+height_rectangle+radius_ball:
            if x_rect - 15 < x_ball + radius_ball <= x_rect or x_rect + width_rectangle <= x_ball - radius_ball <= x_rect + width_rectangle + 15:
                ball_direction(-vx_ball,vy_ball)
                Delete_rectangles.append((j,i))
...
def pause():
    ...
    if keys[pygame.K_ESCAPE]:
        paused_mode = True
        font = pygame.font.Font('UniversCondensed.ttf',40)
...
def endgame():
    ...
    if len(Rectangles) == 0:
        ...
        font = pygame.font.Font('UniversCondensed.ttf',40)
        text = font.render('VICTORY HAS BEEN MADE!!',True,(0,0,0))
    while endgame_mode:
        clock.tick(5)
        screen.blit(text,(300,500))
        keys_local = pygame.key.get_pressed()
